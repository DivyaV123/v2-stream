...
<div
  ...
  <thead>
    <tr class="leading-2">
      ...
      <th class="border  border-indigo-50  bg-gray-800 text-indigo-50 font-semibold text-sm">Status</th>
  ...
  <tbody class="leading-4">
    <tr v-for="(bran, index) in total" :key=index :class="{ 'bg-gray-800': index % 2}"> 
      ...
      <td class="border  border-black  
                 ...
                 font-semibold 
                 text-accent 
                 text-sm">{{ bran.contacts[0].number }}</td>
       <td class="border  border-black  
                  text-center 
                  font-semibold">
             <button  class="
    ...
    <td class="border  border-black  
               ...
               font-semibold 
               text-indigo-300 
               text-sm">{{bran.target}}</td>
    <td class="border  border-black  
               text-center 
               font-semibold 
               text-indigo-300 
               text-sm">{{bran.balance}}</td>
    <td class="border  border-black  
               text-center 
               font-semibold 
               text-indigo-300 
    ...
    <td class="border  border-black  
        	...
        	<button class="inline-flex 
                         ...
                         bg-success 
                         btn-sm
                         " v-if="bran.blocked ==true">Blocked</button><br>
          <button class="inline-flex 
                         justify-center 
                         py-1 
                         px-15 
                         ...
                         font-extrabold 
                         text-black 
                         bg-indigo-300 " v-if="bran.blockedinstance">{{bran.blockedinstance}}</button>
          <!-- <button class="inline-flex 
                         justify-center 
                         py-1 
                         px-12 
      ...
      <td>
      <div class="mb-0  mr-0 text-center pt-1
               ">
           <button  class="
      ...
      <button class="inline-flex 
                     ...
                     text-success
                     bg-black 
                     has-tooltip7" v-if="bran.promiseddate">Promised to pay on {{bran.promiseddate}}
      <br>
      <div class="leading-5 text-indigo-200 uppercase text-center font-extrabold text-xs">
              <span class='tooltip7 shadow-l bg-black text-info mt-4 p-1 rounded-md text-xs border border-info capitalize'>Added by {{bran.promisedauthor}}</span>      
            </div>
            </button>
    </div>
      </td>
                          ...
                          <label for="country" class="block text-sm font-extrabold pb-1 pr-20">Promise date</label>
                            ...
                            <label for="country" class="block text-sm font-extrabold pb-1 pr-20 text-white">Comment</label>
                              ...
                              <div class="border border-white pt-3 pr-4 pl-4 pb-6">
                                  <label for="country" class="block text-sm font-extrabold text-white">Reason</label>
                                          <span
                                          class="
                                              z-10
                                              h-full
 ...
 <Pagination
          ...
          :total="this.$store.state.tracker.total.total"
          :page="page"
...
export default {
  ...
  data(){
    ...
    await this.$store.dispatch('tracker/getTotal',{"page":this.page,"params":this.params,"id":this.$route.query.id})
 ...
 computed: {
    total() 
    {
      return this.$store.state.tracker.total.results
    },
  },
   methods:{
