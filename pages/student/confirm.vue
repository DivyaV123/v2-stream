        ...
        <div class="mb-6 ">
              <div>
                    <ValidationProvider rules="required|min:8|password:@confirm" v-slot="{ errors }">
                      ...
                      />
                      <span class="text-accent">{{ errors[0] }}</span>
                    </ValidationProvider>
              </div>
              <div>
...
export default {
  ...
  async fetch(){
      ...
      const forgot = await this.$axios.get(`student/forgot-password-session/?token=${this.form.token}`);
      if (forgot.data.message==1)
      {
        this.showpop=true
      }
  },
    ...
    async submit() {
      try{
        ...
        const forgot = await this.$axios.post(
          ...
          await this.$store.dispatch("alerts/alert", {
            text: err.response.data.password[0],
            type: "error",
          });
      }
    },
