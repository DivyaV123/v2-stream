...
<div class="text-right mt-0 mr-2 ml-0 px-1 py-1">
  ...
  <tbody class="leading-4">
    ...
    <td class="border  
          ...
          <button class="inline-flex 
                         ...
                         text-xs 
                         font-extrabold 
                         rounded-md text-black  " v-if="bran.updated_v1 ==false" v-on:click="verify=true,rid=bran.id">Add</button>
    </td>
    <td class="border  
             border-black  
               text-center 
 ...
 <Pagination
          ...
          :total="this.$store.state.collector.updated_branch.total"
          :page="page"
...
export default {
  ...
  data(){
    ...
    async fetch(){
    console.log("in fetch",this.params)
    ...
    await this.$store.dispatch('collector/get_updated_branch',{"page":this.page,"params":this.params,"bid":this.$route.query.bid})   
  },
 computed: {
    total() {
      return this.$store.state.collector.updated_branch.results
    },
  },
   methods:{
        maskNumber(numbers)
