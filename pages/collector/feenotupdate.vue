...
<div class="align-middle inline-block shadow sm:rounded-lg">
  ...
  <div class="w-full p-2 m-2 bg-gray-700 rounded flex justify-left ">
      <ol class="flex text-gray-800">
        <li class="font-bold text-success">{{batchess.branch_name}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-white">{{batchess.course_name}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-success">{{batchess.batchcode}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-white">{{batchess.trainer_name}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-success">{{batchess.class_timing_name}}</li>
      </ol>
    </div>
         ...
         <!-- <td class="border  
          ...
          <td class="border  
                ...
                <button class="inline-flex 
                               ...
                               font-extrabold 
                               rounded-md 
                               text-black  " v-if="bran.updated_v1 ==false" v-on:click="verify=true,rid=bran.id,sstudent=bran.studentname">Add</button>
          </td>
          <td class="border  
                   border-black  
                     text-center 
 ...
 <Pagination
          ...
          :total="this.$store.state.collector.notupdated.total"
          :page="page"
...
export default {
  ...
  data(){
    ...
    await this.$store.dispatch('collector/get_notupdated',{"page":this.page,"params":this.params,"id":this.$route.query.id})
 ...
 computed: {
    total() {
      return this.$store.state.collector.notupdated.results
    },
  },
   methods:{
        maskNumber(numbers)
      ...
      async updateSubmit(){
       ...
       await this.$store.dispatch("alerts/alert", {
          text: "Successfully Added the student "+this.sstudent,
          type: "success",
        });
