  ...
  <section class="mr-4">
    <div class="mt-1 mx-1">
      ...
      <table class="border-collapse border border-black border-3 border-opacity-50  w-full table-auto m-3 mr-36">
        <thead>
          <tr class="leading-2">
            ...
            <th class="border  border-black  bg-blue-100  text-black font-semibold text-sm">Verified in V1</th>
          ...
          <tbody class="leading-4">
            <tr v-for="(bran, index) in total" :key=index :class="{ 'bg-gray-800': index % 2}"> 
              <td class="border  border-black  text-left font-semibold pt-2 text-sm text-white">
                <span class="uppercase font-mono ml-1">{{ bran.firstname }}</span> <br>
                <span class="text-gray-400 ml-1">{{ bran.studentname }}</span>  
              </td>
              <td class="border  border-black  text-center font-semibold text-green-200 text-sm">
              ...
              <td class="border  border-black  text-center font-semibold text-blue-400 pb-2">
                <button class="inline-flex justify-center py-1 px-15 border border-black shadow-sm text-xs font-extrabold rounded-md text-black bg-info btn-sm" v-if="bran.verified ==true">
                  Verified
                </button><br>
                <button class="inline-flex justify-center py-1 px-15 border border-transparent shadow-sm text-xs font-extrabold text-black bg-white rounded:full " v-if="bran.verified ==true">
                  Addded in v1 by-{{bran.verified_by}}
                </button>
                <button class="inline-flex justify-center py-1 px-12 border border-black bg-warning  shadow-sm text-xs font-extrabold rounded-md text-black" v-if="bran.verified ==false" v-on:click="verify=true,rid=bran.id">
                  Verify
                </button>
              </td>
              <td class="border  border-black  text-center font-semibold text-blue-200 text-sm">
                {{bran.branch}}
              </td>
              <td class="border  border-black  text-center font-semibold text-blue-200 text-sm">
                {{bran.batches}}
      ...
      <Pagination
            ...
            :total="this.$store.state.oh.verified.total"
            :page="page"
...
export default {
  ...
  async fetch(){
    await this.$store.dispatch('oh/getVerified',{"page":this.page,"params":this.params,"id":this.$route.query.id,"bid":this.$route.query.bid})
  ...
  computed: {
    total() {
      return this.$store.state.oh.verified.results
    }
  },
  methods:{
    maskNumber(numbers){
