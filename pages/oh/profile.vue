<template>
  <section class="mr-4">
    ...
    <Pagination
      ...
      :total="this.$store.state.oh.profile.total"
      :page="page"
...
export default {
  head () {
    return {
      title: 'Profile added student List | OH',
    }
  },
  ...
  async fetch(){
    ...
    await this.$store.dispatch('oh/getProfile',{"page":this.page,"params":this.params,"id":this.$route.query.id,"bid":this.$route.query.bid})
    // else{
    //   this.searchParams()
    // }
  },
  computed: {
    total() {
      return this.$store.state.oh.profile.results
    },
  },
  methods:{
    maskNumber(numbers){
