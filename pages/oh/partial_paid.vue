  ...
  <section class="mr-4">
    <div class="mt-1 mx-1">
              ...
              <form action="#" method="POST" @submit.stop.prevent="handleSubmit(searchParams)">
                <div class="grid md:grid-cols-5 sm:grid-cols-1 gap-5 mt-0 ml-2">
                  ...
                  <div>
                    ...
                    <div>
                      <input type="text" name="number" id="number" autocomplete="number" class="mt-1 pt-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md  text-black placeholder-gray-500"  v-model="search.number" >
                    </div>
                  </div>
                  <div>
      ...
      <Pagination
          ...
          :total="this.$store.state.oh.partial.total"
          :page="page"
...
export default {
  ...
  async fetch(){
    ...
    await this.$store.dispatch('oh/getPartial',{"page":this.page,"params":this.params,"id":this.$route.query.id,"bid":this.$route.query.bid})
    // else{
    //   this.searchParams()
    // }
  },
  computed: {
    total() {
      return this.$store.state.oh.partial.results
    },
