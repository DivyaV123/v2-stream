    ...
    <!-- <validation-observer ref="observer" v-slot="{ handleSubmit }">
      ...
      > -->
        <div class="w-full flex flex-wrap justify-center">
          ...
          <div>
            <input type="text" name="number" id="number" autocomplete="number" class="mt-1  pt-3 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md  text-black placeholder-gray-500 "  v-model="count" placeholder="Search batch count">
          </div>
          <div class="flex mt-1 ml-1">
            <button class="btn btn-sm btn-success" @click="searchByDate()">Search</button><br>
            <button class="btn btn-sm btn-glass text-success border border-success" @click="cleardata()">Clear</button>
          </div>
        </div>
      <!-- </form>
    </validation-observer> -->
          ...
          <tr v-for="(s, i) in students" :key="i">
            <td>
              <span
                ...
                v-if="s.blocked"
                @click="unblockStudent(s.id, s.student, s.name)"
                >Un-Block</span
              ><span
                class="btn btn-xs btn-warning"
                v-else
                @click="blockStudent(s.id, s.student, s.name)"
                >Block</span
              >
            </td>
            <td>{{ s.name }}</td>
            ...
            <td>
              <span class="btn btn-xs bg-white border border-black text-black">{{ s.total_batches }}</span>
            </td>
            <td>
              {{ s.batchcode }}
            </td>
...
export default {
  ...
  head() {
    ...
    if (this.$route.query.type == "si") {
      title = "Signed-Up Students | Corporate"
    }
    else if(this.$route.query.type == "pl"){
      title = "Placement Form Students | Corporate"
    }
    else if(this.$route.query.type == "npl"){
      title = "No Placement Form Students | Corporate"
    }
    else if(this.$route.query.type == "sf"){
      title = "Allocation Form Students | Corporate"
    }
    else if(this.$route.query.type == "nsf"){
      title = "No Allocation Form Students | Corporate"
    }
    else if (this.$route.query.type == "ve"){
      title = "Verified Students | Corporate"
    }
    else {
      title = "Not Verified Students | Corporate"
    }
    return {
      title: title
    };
