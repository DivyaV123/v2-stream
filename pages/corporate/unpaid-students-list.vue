...
<div  class="align-middle inline-block shadow sm:rounded-lg">
  <div class="text-center uppercase font-serif text-3xl text-success mb-2">
      Batch Allocation Unpaid Students List
    </div>
...
export default {
  ...
  data(){
    ...
    await this.$store.dispatch('student/getEnquiryForm1',{"page":this.page,"params":this.params})
    await this.$store.dispatch('student/getFormReason')
    let date = dayjs(new Date()).format("YYYY-MM-DD")
    this.total = this.$store.state.student.enquiry_form_list1.total
 ...
 computed: {
    batches() 
    {
      console.log("count",this.$store.state.student.enquiry_form_list1.results)
      return this.$store.state.student.enquiry_form_list1.results
    }, 
  },
   methods:{
        async confirmClose(){
          this.closebatch = false;
          await this.$store.dispatch('student/getUpdate',{"id":this.id1,"comment":this.comment, "status":this.status});
