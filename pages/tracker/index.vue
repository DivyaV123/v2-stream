...
<div class="align-middle inline-block shadow overflow-x-auto sm:rounded-lg">
    ...
    <tr v-for="(bran, index) in branches" :key=index > 
     ...
     <td class="px-2 
            ...
            <NuxtLink class="hover:text-black-500" 
                      :to="bran.batches > 0 ?`/tracker/batches?id=${bran.id}`:''"  
                      :target="bran.batches > 0 ? '_blank' : ''"  >
                <div class="leading-5 text-indigo-200 uppercase text-center font-extrabold ">
      ...
      <td class="border  
               border-indigo-50  
                 text-center 
                 font-semibold 
                 text-blue-200">{{bran.notupdatedv1}}
      </td>
    </tr>
   </tbody>  
   </template>
 ...
 <Pagination
          ...
          :total="this.$store.state.tracker.branches.total"
          :page="page"
...
export default {
  ...
  data(){
    ...
    await this.$store.dispatch('tracker/getBranch',{"page":this.page,"params":this.params})
  },
 computed: {
    branches() {
      return this.$store.state.tracker.branches.results
    },
  },
   methods:{
    onPageChange(page) {
