...
<div class="text-right mt-0 mr-2 ml-0 px-1 py-1">
  ...
  <tbody class="leading-4 hover:bg-gray-900">
    <tr v-for="(bran, index) in batches" :key=index :class="{ 'bg-gray-800': index % 2}"> 
     ...
     <td class="px-2 
          ...
          <NuxtLink class="hover:text-black-500" 
                    :to="bran.total > 0 ?`/tracker/total?id=${bran.id}&date=${date.date}`:''" 
    ...
    <td class="px-2 
               ...
               has-tooltip7" >
          <NuxtLink class="hover:text-black-500" 
                    :to="bran.attended > 0 ?`/tracker/total-attended?id=${bran.id}&date=${date.date}`:''" 
    ...
    <td class="px-2 
               ...
               has-tooltip7" >
            <NuxtLink class="hover:text-black-500" 
                      :to="bran.nattended > 0 ?`/tracker/total-notattended?id=${bran.id}&date=${date.date}`:''"  
     ...
     <td class="px-2 
                ...
                has-tooltip7" >
          <NuxtLink class="hover:text-black-500" 
                    :to="bran.promised > 0 ?`/tracker/total-promised?id=${bran.id}&date=${date.date}`:''" 
    ...
    <td class="border  border-black  
     ...
     <td class="px-2 
                 ...
                 has-tooltip7" >
            <NuxtLink class="hover:text-black-500" 
                      :to="`/tracker/total-notpaid?id=${bran.id}&date=${date.date}`"  
                      target='_blank' >
            <div class="leading-5 text-indigo-200 uppercase text-center font-extrabold text-xs">
     ...
     <td class="px-2 
                ...
                has-tooltip7" >
            <NuxtLink class="hover:text-black-500" 
                      :to="`/tracker/total-partialpaid?id=${bran.id}&date=${date.date}`" 
                      target='_blank' >
            <div class="leading-5 text-indigo-200 uppercase text-center font-extrabold text-xs">
                <span class='tooltip7 shadow-l bg-yellow-600 text-white mt-4 p-1 rounded-md text-sm'>click</span>
                {{bran.partial}}
    ...
    <td class="px-2 
     ...
     <td class="px-2 
                ...
                has-tooltip7" >
       <NuxtLink class="hover:text-black-500" 
                 :to="bran.notupdatedv1 > 0 ?`/tracker/fee_notupdated?id=${bran.id}&date=${date.date}`:''"  
 ...
 <Pagination
          ...
          :total="this.$store.state.tracker.batches.total"
          :page="page"
...
export default {
  ...
  data(){
    ...
    async fetch(){
    await this.$store.dispatch('tracker/getBatch',{"page":this.page,"params":this.params,"id":this.$route.query.id})
 ...
 computed: {
    batches() 
    {
      return this.$store.state.tracker.batches.results
    }, 
  },
   methods:{
            searchStream(search, loading) {
