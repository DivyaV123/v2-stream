...
<button @click="isOpen = !isOpen" class="text-xs bg-white py-1 px-2 rounded text-black align-middle cursor-default border border-black font-extrabold">Search</button>
  ...
  <thead>
    <tr class="leading-6">
      <th class="border  border-white  bg-gray-400 text-black ">Batchcode</th>
    ...
    <tr v-for="(bran, index) in branches" :key=index :class="{ 'bg-gray-800': index % 2}"> 
      <td class="border  
               border-indigo-50  
                 text-center 
                 font-semibold 
                 text-gray-100">{{bran.batchcode}}</td>
      <td class="px-2 
                 py-1 
                 whitespace-nowrap 
              ...
              <NuxtLink class="hover:text-black-500" 
                        :to="bran.notupdatedv1 > 0 ?`/verifier/batch_notupdated?id=${bran.id}&date=${date.date}`:''"  
        ...
        <td class="px-2 
                   ...
                   has-tooltip7" >
              <NuxtLink class="hover:text-black-500" 
                        :to="bran.updatedv1 > 0 ?`/verifier/batch_updated?id=${bran.id}&date=${date.date}`:''" 
        ...
        <td class="px-2 
                   ...
                   text-center" >
                <NuxtLink class="hover:text-black-500" 
                          :to="bran.verified > 0 ?`/verifier/batch_verified?id=${bran.id}&date=${date.date}`:''" 
           ...
           <td class="px-2 
                      ...
                      text-center" >
                    <NuxtLink class="hover:text-black-500 " 
                              :to="bran.notverified > 0 ?`/verifier/batch_notverified?id=${bran.id}&date=${date.date}`:''"  
 ...
 <Pagination
          ...
          :total="this.$store.state.verifier.batch.total"
          :page="page"
...
export default {
  ...
  data(){
    ...
    await this.$store.dispatch('verifier/getBatch',{"page":this.page,"params":this.params,"id":this.$route.query.id})
 ...
 computed: {
    branches() {
      return this.$store.state.verifier.batch.results
    },
  },
   methods:{  
        onPageChange(page) 
