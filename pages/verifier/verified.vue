...
<div class="text-right mt-0 mr-2 ml-0 px-1 py-1">
  ...
  <tbody class="leading-4">
    <tr v-for="(bran, index) in total" :key=index :class="{ 'bg-gray-800': index % 2}"> 
      <td class="border  border-black  text-left font-semibold pt-2 text-sm text-white">{{bran.studentname}}   
      </td>
      <td class="border  border-black  text-center font-semibold text-green-200 text-sm">{{ bran.contacts[0].number }}</td>
      ...
      <td class="border  border-black  text-center font-semibold text-blue-400 pb-2">
            <button class="
              ...
              bg-info 
              btn-sm" 
              v-if="bran.verified ==true">Verified
            </button><br>
            <button class="
              inline-flex 
              justify-center 
              ...
              bg-white
               rounded:full " 
              v-if="bran.verified ==true">Addded in v1 by-{{bran.verified_by}}
              </button>
            <button class="
            inline-flex 
            justify-center 
            ...
            rounded-md 
            text-black" 
            v-if="bran.verified ==false" 
            v-on:click="verify=true,rid=bran.id">Verify
            </button>
      </td>
      <td class="
          border  border-black  
 ...
 <Pagination
          ...
          :total="this.$store.state.verifier.verified.total"
          :page="page"
...
export default {
  ...
  data(){
    ...
    await this.$store.dispatch('verifier/getVerified',{"page":this.page,"params":this.params,"bid":this.$route.query.bid})
    // else{
    //   this.searchParams()
    // }     
  },
computed: {
    total() {
      return this.$store.state.verifier.verified.results
    },
  },
methods:{
        maskNumber(numbers){
