...
<div class="align-middle inline-block shadow sm:rounded-lg">
  ...
  <div class="w-full p-2 m-2 bg-gray-700 rounded flex justify-left ">
      <ol class="flex text-gray-800">
        <li class="font-bold text-yellow-500">{{batchess.branch_name}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-white">{{batchess.course_name}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-yellow-500">{{batchess.batchcode}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-white">{{batchess.trainer_name}}</li>
        <li><span class="mx-2 font-bold text-white">|</span></li>
        <li class="font-bold text-yellow-500">{{batchess.class_timing_name}}</li>
      </ol>
    </div>
<div class="text-right mt-0 mr-2 ml-0 px-1 py-1">
       ...
       <td class="border  
        ...
        <td class="border  
             ...
             <button  class="
                    ...
                    text-xs
                    btn btn-glass
                    border border-warning">{{bran.date}}
              </button>
        </td>
         <!-- <td class="border  
                  border-black  
         ...
         <td class="border  
             ...
             <button  class="
                    ...
                    text-xs
                    btn btn-glass
                    border border-error">{{bran.target}}
              </button>
          </td>
          <td class="border  
                   border-black  
                     ...
                     pb-2">
        	          <button class="inline-flex 
                                   ...
                                   text-black 
                                   bg-info 
                                   btn-sm" v-if="bran.verified ==true">Verified</button><br>
                    <button class="inline-flex 
                                   justify-center 
                                   py-1 
                                   px-15 
                                   ...
                                   text-black 
                                   bg-white 
                                   rounded:full " v-if="bran.verified ==true">Addded in v1 by-{{bran.verified_by}}</button>
                    <button class="inline-flex 
                                   justify-center 
                                   py-1 
                                   px-12 
                                   ...
                                   font-extrabold 
                                   rounded-md 
                                   text-black  " v-if="bran.verified ==false" v-on:click="verify=true,rid=bran.id,sstudent=bran.studentname">Verify</button>
          </td>
          <td class="border  
                   border-black  
                     text-center 
 ...
 <Pagination
          ...
          :total="this.$store.state.verifier.batch_notverified.total"
          :page="page"
...
export default {
   head () {
    return {
      title: 'Not Verified List | Verifier',
    }
  },
  ...
  data(){
    ...
    this.batchess = batch.data.results[0]
    ...
    await this.$store.dispatch('verifier/getBatchNotVerified',{"page":this.page,"params":this.params,"id":this.$route.query.id})
    // else
    // {
    //   this.searchParams()
    // }     
 ...
 computed: {
    total() {
      return this.$store.state.verifier.batch_notverified.results
    },
  },
   methods:{
        maskNumber(numbers)
        ...
        await this.$store.dispatch("alerts/alert", {
            text: "Successfully Verified the student "+this.sstudent,
            type: "success",
          });
